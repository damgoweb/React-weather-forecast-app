...
const WeatherForecast = () => {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      ...
      const url = `https://api.openweathermap.org/data/2.5/weather?q=${sanitizedCity}&appid=${API_KEY}&units=metric`;
  ...
  const handleCityChange = (event) => {
    const sanitizedCity = sanitizeInput(event.target.value);
